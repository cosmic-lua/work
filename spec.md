...
Batch 2 of the all-runner migration (parent: 3IOCdooE): every test file under
`_make/`, `_cli/`, `_tool/`, `_build/` and `_types/` is `runner` mode — the self-call lines are
...
- File counts at capture: _make 31, _cli 19, _tool 15, _build 12,
  _types 8 → 85 files. Self-call lines: `grep -h "^test_"
  $(git ls-files '_make/**' '_cli/**' '_tool/**' '_build/**'
  '_types/**' | grep '_test.tl$') | wc -l` (re-measure at pull; tree
  total was 2,943 across 259 files).
...
One commit, deletions only in the five trees' `*_test.tl` files: for each
...
owns the prose flip). `cosmic/**` (batch 1) and the rest (batch 3) untouched.

## Acceptance

- Before the edit, record `bin/cosmic --make test _make _cli _tool _build _types | tail -3`'s
  check count; after, the same command reports the SAME number of
  test functions run and `test: PASS`.
- `bin/cosmic --make ci` ends `ci: PASS`.
- the same grep over the five trees → 0, or exactly the calls of files
  the PR body lists as left-legacy.
- `git diff --numstat origin/main` shows only the five trees'
  `*_test.tl` rows, 0 additions each (pure deletions), except `.cosmic-coverage`
