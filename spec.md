...
G4, under the tree-migration container (3IOCdooE): batch 4 of seven —
the cosmic/ subdirectories: sqlite, quicksand, fetch, net, doc lose their test self-call lines, so those files run because
...
- **Scope**: `cosmic/sqlite/`, `cosmic/quicksand/`, `cosmic/fetch/`, `cosmic/net/`, `cosmic/doc/`.
- **414 self-call lines across 38 files**, counted with the
  same pattern the edit deletes:

  ```
  grep -rln '^test_[A-Za-z0-9_]*()$' --include='*_test.tl' cosmic/sqlite cosmic/quicksand cosmic/fetch cosmic/net cosmic/doc | xargs grep -hc '^test_[A-Za-z0-9_]*()$' | paste -sd+ | bc
  ```
...
```
grep -rln '^test_[A-Za-z0-9_]*()$' --include='*_test.tl' cosmic/sqlite cosmic/quicksand cosmic/fetch cosmic/net cosmic/doc | xargs sed -i -E '/^test_[A-Za-z0-9_]*\(\)$/d'
```
...
- No self-call survives in scope:
  `grep -rln '^test_[A-Za-z0-9_]*()$' --include='*_test.tl' cosmic/sqlite cosmic/quicksand cosmic/fetch cosmic/net cosmic/doc | xargs grep -c '^test_[A-Za-z0-9_]*()$' | grep -v ':0$'`
