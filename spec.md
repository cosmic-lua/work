...
## Ended 2026-08-19: the board migration answered this structurally

Every artifact this item's evidence names is label-era and gone —
`_work/model.tl` (0 matches in `_work/` today), the `work:*` labels, the
phase-table rows and planner rule lists that drifted. The migrated
system removed the drift CLASS instead of gating it: the skill now
deliberately restates nothing the tool defines — "the verbs are the
tool's to describe, not this skill's" (`skills/work/SKILL.md:74`), verb
listings and options are generated from the CLI (`gitboard help`, board
`README.md:49`: "generated from the CLI, so neither can drift"), and the
WIP numbers live only in `_work/flow.tl`'s `LIMITS`, printed by
`status`. What prose remains states what the verbs are FOR — judgment,
which is what a gate cannot pin. A future drift between skill prose and
tool behavior is fresh evidence for a fresh item against the new system,
not this one.
