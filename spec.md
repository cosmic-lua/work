...
G4, under the tree-migration container (3IOCdooE): batch 7 of seven —
cosmic/*_test.tl, stream_test.tl through zip_test.tl lose their test self-call lines, so those files run because
...
- **Scope**: the top-level `cosmic/*_test.tl` files whose names sort from `stream_test.tl` through `zip_test.tl` inclusive (25 files at measurement).
- **406 self-call lines across 25 files**, counted with the
  same pattern the edit deletes:

  ```
  ls cosmic/*_test.tl | awk '$0 >= "cosmic/stream_test.tl" && $0 <= "cosmic/zip_test.tl"' | xargs grep -hc '^test_[A-Za-z0-9_]*()$' | paste -sd+ | bc
  ```
...
- No self-call survives in scope:
  `ls cosmic/*_test.tl | awk '$0 >= "cosmic/stream_test.tl" && $0 <= "cosmic/zip_test.tl"' | xargs grep -c '^test_[A-Za-z0-9_]*()$' | grep -v ':0$'`
