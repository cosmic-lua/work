...
G4, under the tree-migration container (3IOCdooE): batch 6 of seven —
cosmic/*_test.tl, html_test.tl through sse_test.tl lose their test self-call lines, so those files run because
...
- **Scope**: the top-level `cosmic/*_test.tl` files whose names sort from `html_test.tl` through `sse_test.tl` inclusive (20 files at measurement).
- **404 self-call lines across 20 files**, counted with the
  same pattern the edit deletes:

  ```
  ls cosmic/*_test.tl | awk '$0 >= "cosmic/html_test.tl" && $0 <= "cosmic/sse_test.tl"' | xargs grep -hc '^test_[A-Za-z0-9_]*()$' | paste -sd+ | bc
  ```
...
```
ls cosmic/*_test.tl | awk '$0 >= "cosmic/html_test.tl" && $0 <= "cosmic/sse_test.tl"' | xargs sed -i -E '/^test_[A-Za-z0-9_]*\(\)$/d'
```
...
- No self-call survives in scope:
  `ls cosmic/*_test.tl | awk '$0 >= "cosmic/html_test.tl" && $0 <= "cosmic/sse_test.tl"' | xargs grep -c '^test_[A-Za-z0-9_]*()$' | grep -v ':0$'`
