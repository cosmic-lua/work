...
`refs/board/lanes`, `refs/board/seq` and `refs/board/format` hold
board-wide state the same way, outside the per-item namespace.
...
and anything the store's own tolerant decode had to flag.
