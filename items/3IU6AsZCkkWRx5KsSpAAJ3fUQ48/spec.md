...
the cosmic/ subdirectories: fs, child, format, flags, coverage, sandbox lose their test self-call lines, so those files run because
...
- **Scope**: `cosmic/fs/`, `cosmic/child/`, `cosmic/format/`, `cosmic/flags/`, `cosmic/coverage/`, `cosmic/sandbox/`.
- **338 self-call lines across 28 files**, counted with the
  same pattern the edit deletes:

  ```
  grep -rln '^test_[A-Za-z0-9_]*()$' --include='*_test.tl' cosmic/fs cosmic/child cosmic/format cosmic/flags cosmic/coverage cosmic/sandbox | xargs grep -hc '^test_[A-Za-z0-9_]*()$' | paste -sd+ | bc
  ```
...
```
grep -rln '^test_[A-Za-z0-9_]*()$' --include='*_test.tl' cosmic/fs cosmic/child cosmic/format cosmic/flags cosmic/coverage cosmic/sandbox | xargs sed -i -E '/^test_[A-Za-z0-9_]*\(\)$/d'
```
...
- No self-call survives in scope:
  `grep -rln '^test_[A-Za-z0-9_]*()$' --include='*_test.tl' cosmic/fs cosmic/child cosmic/format cosmic/flags cosmic/coverage cosmic/sandbox | xargs grep -c '^test_[A-Za-z0-9_]*()$' | grep -v ':0$'`
