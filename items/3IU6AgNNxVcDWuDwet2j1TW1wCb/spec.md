...
the dispatcher and the build system lose their test self-call lines, so those files run because
...
- **Scope**: `_cli/`, `_make/`.
- **483 self-call lines across 50 files**, counted with the
  same pattern the edit deletes:

  ```
  grep -rln '^test_[A-Za-z0-9_]*()$' --include='*_test.tl' _cli _make | xargs grep -hc '^test_[A-Za-z0-9_]*()$' | paste -sd+ | bc
  ```
...
```
grep -rln '^test_[A-Za-z0-9_]*()$' --include='*_test.tl' _cli _make | xargs sed -i -E '/^test_[A-Za-z0-9_]*\(\)$/d'
```
...
- No self-call survives in scope:
  `grep -rln '^test_[A-Za-z0-9_]*()$' --include='*_test.tl' _cli _make | xargs grep -c '^test_[A-Za-z0-9_]*()$' | grep -v ':0$'`
