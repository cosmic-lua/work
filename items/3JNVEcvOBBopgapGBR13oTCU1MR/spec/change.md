...
file, to go run the in-place formatter by hand. There is no bulk write mode
...
continuation indent — mechanical, deterministic, exactly what the in-place formatter emits.
Roughly 15-25 minutes of wall clock and two extra full-suite runs for zero
information the formatter did not already have. On the fifth item I formatted the changed files in place BEFORE the gate and `fmt` passed first time; that is the
control.
