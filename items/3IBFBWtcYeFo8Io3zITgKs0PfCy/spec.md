...
- No `.cosmic-coverage` row for the new modules — this item lands the
  code; the coverage baseline gets rewritten by whoever runs
  `--make coverage --baseline` next, in its own commit. The last
  attempt (PR #1297) added the row inside this item's diff, which
  couples this item to a floor rewrite that is not its concern.
...
- No `.cosmic-coverage` row inside this item's diff.
